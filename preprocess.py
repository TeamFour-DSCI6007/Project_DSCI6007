...
def preprocess_tbnet():
    ...
    for d in eval_ds.create_dict_iterator():
        item_rst = d['item'].asnumpy().astype(int)
        rl1_rst = np.expand_dims(d['relation1'].asnumpy().astype(int), axis=0)
        ety_rst = np.expand_dims(d['entity'].asnumpy().astype(int), axis=0)
        rl2_rst = np.expand_dims(d['relation2'].asnumpy().astype(int), axis=0)
        his_rst = np.expand_dims(d['hist_item'].asnumpy().astype(int), axis=0)
